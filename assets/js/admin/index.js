// CAMBIAR EN SERVER
const pathInicial = "/tresolqa/"; // cambiar por '/'
const baseUrl = location.origin + pathInicial;
	...
	$.ajax({
		...
		success: function (resp) {
			...
			if (data.status === "ok") {
        		...
        		location.href = "<?= base_url(EQUIPO_GAMER_PATH . '/indexNuevoEquipoGamer') ?>"
			} else {
				mostrarErrors(data.title, data.errors);
			}
		},
...
function subirFoto2(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = function (e) {
           ...
           if (fileType === 'pdf') {
                $("#foto").attr("src", baseUrl + "assets/img/polizas/pdf.png");
            } else if (fileType === 'doc' || fileType === 'docx') {
                $("#foto").attr("src", baseUrl + "assets/img/polizas/docx.png");
            } else {
        ...
        reader.onerror = function (e) {
            ...
            $("#foto").attr("src", baseUrl + "assets/img/polizas/no-image.jpg");
        };
...
function validarFile2(all) {
	$("#noFoto").addClass("d-none");
	//EXTENSIONES Y TAMANO PERMITIDO.
	let extensiones_permitidas = [".png", ".bmp", ".jpg", ".jpeg", ".pdf", ".doc", ".docx"];
	//let tamano = 8; // EXPRESADO EN MB.
	let rutayarchivo = all.value;
	let ultimo_punto = all.value.lastIndexOf(".");
	let extension = rutayarchivo.slice(ultimo_punto, rutayarchivo.length);
